...
function Stat({ label, value, suffix = "", accent = false }: { label: string; value: number; suffix?: string; accent?: boolean }) {
  return (
    ...
    >
      <div style={{ fontFamily: "var(--font-display)", fontSize: 22, fontWeight: 700, color: accent ? "var(--accent-2)" : "var(--text)", letterSpacing: "-0.02em" }}>
        {value}{suffix}
      </div>
...
const funnel = [
  { l: "Registered", v: 100, c: "var(--accent)" },
  { l: "Recruiter Scans", v: 36, c: "var(--accent-2)" },
  { l: "Shortlisted", v: 15, c: "var(--accent-2)" },
  { l: "Interviews", v: 5, c: "var(--text-2)" },
  { l: "Offers", v: 3, c: "var(--text-2)" },
];
        ...
        <div className="reveal-grid" style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr 1.1fr", gap: 12 }}>
          ...
          <Panel title="QR Check-in">
            <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 12 }}>
              <div style={{ position: "relative", width: 56, height: 56, background: "#0A0A0A", borderRadius: 8, padding: 5, display: "grid", gridTemplateColumns: "repeat(6,1fr)", gap: 2, overflow: "hidden", border: "1px solid var(--border-strong)" }}>
                ...
                <div className="scan-line" style={{ position: "absolute", left: 0, right: 0, top: 0, height: 14, background: "linear-gradient(180deg, transparent, rgba(255,255,255,0.55), transparent)" }} />
              </div>
              <div>
