...
const pipeline = [
  ...
  { l: "Shortlisted", v: 74, c: "var(--teal)", pct: 41 },
  { l: "Contacted", v: 48, c: "var(--cyan)", pct: 27 },
  { l: "Interview", v: 26, c: "#2BB8E8", pct: 14 },
  { l: "Offer", v: 14, c: "var(--amber)", pct: 8 },
];
...
const candidates = [
  ...
  { n: "Fatima Khalid", d: "Marketing · Year 3", r: 78, res: 82, s: "Interview", tone: "amber" as const },
...
export default function EmployerCRMSection() {
  return (
    <Section id="employers" bg="var(--bg-2)">
          ...
          <div>
            <GlassCard padding={22}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {filters.map((f) => (
                  <span key={f} style={{ fontSize: 12, color: "var(--text-2)", background: "rgba(53,211,255,0.06)", border: "1px solid var(--border)", borderRadius: "var(--r-full)", padding: "5px 12px", cursor: "pointer", transition: "all 0.18s" }} className="filter-chip">{f}</span>
                ))}
              </div>
            </GlassCard>
          </div>
        ...
        <div>
          <GlassCard padding={0} style={{ overflow: "hidden", border: "1px solid var(--border-strong)" }}>
            <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 600, color: "var(--cyan)", background: "rgba(53,211,255,0.08)", border: "1px solid var(--border-strong)", borderRadius: "var(--r-sm)", padding: "6px 12px", cursor: "pointer" }}>Export CSV</span>
            </div>
            {candidates.map((c, i) => (
              <div key={c.n} className="cand-row" style={{ padding: "16px 20px", borderBottom: i < candidates.length - 1 ? "1px solid var(--border)" : "none", transition: "background 0.18s" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  <div style={{ display: "flex", gap: 11, alignItems: "center" }}>
                    <div style={{ width: 36, height: 36, borderRadius: "50%", background: "rgba(53,211,255,0.10)", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "var(--cyan)", fontFamily: "var(--font-display)" }}>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {actions.map((a) => (
                    <button key={a} className="cand-action" style={{ fontSize: 11, fontWeight: 600, color: a === "Shortlist" ? "var(--teal)" : "var(--text-2)", background: a === "Shortlist" ? "rgba(0,194,168,0.10)" : "rgba(255,255,255,0.04)", border: `1px solid ${a === "Shortlist" ? "rgba(0,194,168,0.25)" : "var(--border)"}`, borderRadius: "var(--r-sm)", padding: "6px 11px", cursor: "pointer", transition: "all 0.18s" }}>{a}</button>
                  ))}
                </div>
              </div>
            ))}
      ...
      <style>{`
        .filter-chip:hover { color: var(--cyan) !important; border-color: var(--border-strong) !important; }
