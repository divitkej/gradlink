  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
        <div style={{ width: 44, height: 44, borderRadius: "50%", background: "rgba(53,211,255,0.12)", border: "1px solid var(--border-strong)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--cyan)", fontWeight: 700, fontFamily: "var(--font-display)" }}>SA</div>
        <div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        {["Resume ✓", "Portfolio ✓", "LinkedIn ✓"].map((l) => (
          <span key={l} style={{ flex: 1, fontSize: 11, textAlign: "center", color: "var(--teal)", background: "rgba(0,194,168,0.08)", border: "1px solid rgba(0,194,168,0.2)", borderRadius: "var(--r-sm)", padding: "6px 4px" }}>{l}</span>
        ))}
      </div>
    </div>
  );
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {[["Mock Interview", "Pending"], ["Resume Workshop", "Recommended"], ["Add 3 Projects", "Pending"]].map(([l, s]) => (
          <div key={l} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, padding: "7px 10px", background: "rgba(255,255,255,0.03)", border: "1px solid var(--border)", borderRadius: "var(--r-sm)" }}>
            ...
            <span style={{ color: s === "Pending" ? "var(--amber)" : "var(--cyan)", fontWeight: 600 }}>{s}</span>
          </div>
        ))}
      </div>
    </div>
...
function MiniLive() {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
        <div style={{ position: "relative", width: 54, height: 54, background: "#02101a", borderRadius: 8, padding: 5, display: "grid", gridTemplateColumns: "repeat(6,1fr)", gap: 2, overflow: "hidden", border: "1px solid var(--border-strong)" }}>
          {Array.from({ length: 36 }).map((_, i) => (
            <div key={i} style={{ background: [0,1,6,7,4,5,10,24,25,30,14,21,28,35,18,13].includes(i) ? "var(--cyan)" : "transparent", borderRadius: 1 }} />
          ))}
          <div className="scan-line" style={{ position: "absolute", left: 0, right: 0, top: 0, height: 14, background: "linear-gradient(180deg, transparent, rgba(53,211,255,0.5), transparent)" }} />
        </div>
        <div>
        ...
        <div style={{ marginLeft: "auto" }}><Badge tone="amber" pulse>Live</Badge></div>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {[["09:00", "Resume Workshop", true], ["10:30", "PwC Booth", true], ["11:00", "Mock Interview", false]].map(([t, l, done]) => (
          <div key={t as string} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12 }}>
            ...
            {done ? <Check size={14} color="var(--teal)" /> : <span style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--border)" }} />}
          </div>
        ))}
      </div>
    </div>
...
function MiniCRM() {
  const rows = [["Sara Al Rashidi", "Shortlisted", "var(--teal)"], ["M. Al Mansoori", "Contacted", "var(--cyan)"], ["Fatima Khalid", "Interview", "var(--amber)"]];
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 6, marginTop: 4 }}>
        {[["Scanned", 180], ["Short", 74], ["Intvw", 26], ["Offers", 14]].map(([l, v]) => (
          <div key={l as string} style={{ textAlign: "center", padding: "8px 2px", background: "rgba(53,211,255,0.06)", borderRadius: "var(--r-sm)", border: "1px solid var(--border)" }}>
            <div style={{ fontFamily: "var(--font-display)", fontSize: 15, fontWeight: 700, color: "var(--cyan)" }}>{v}</div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  boxShadow: isActive ? "0 0 30px rgba(53,211,255,0.12)" : "none",
                }}
              >
                <div
                  style={{
                    ...
                    background: isActive ? "linear-gradient(135deg, var(--cyan), var(--teal))" : "rgba(255,255,255,0.04)",
                ...
                >
                  <MIcon size={20} color={isActive ? "#021016" : "var(--text-2)"} strokeWidth={1.7} />
                </div>
                <div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                <div style={{ width: 40, height: 40, borderRadius: "var(--r-md)", background: "linear-gradient(135deg, var(--cyan), var(--teal))", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <Icon size={20} color="#021016" strokeWidth={1.7} />
                </div>
              ...
              <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 9, marginBottom: 24 }}>
                {m.bullets.map((b) => (
                  <li key={b} style={{ display: "flex", alignItems: "center", gap: 9, fontSize: 14, color: "var(--text-2)" }}>
                    <Check size={15} color="var(--teal)" strokeWidth={2.2} />
                    {b}
                  </li>
                ))}
              </ul>
